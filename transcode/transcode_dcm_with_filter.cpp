...
static int open_input_file(const char* filename)
{
    ...
    /*const AVDictionary* dict = ifmt_ctx->metadata;*/
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO
            ...
            if (dec_ctx && (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO
                ...
                out_stream->avg_frame_rate = dec_ctx->framerate;
...
static int init_filters(void)
{
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (ifmt_ctx->streams[i]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
            filter_spec = "yadif"; /* passthrough (dummy) filter for video */
        else
...
static int encode_write_frame(unsigned int stream_index, int flush)
{
    ...
    while (ret >= 0) {
        ...
        if (filt_frame && enc_pkt->dts != AV_NOPTS_VALUE) {
            enc_pkt->dts = av_rescale_q(enc_pkt->dts, filt_frame->time_base, stream->enc_ctx->time_base);
        }
...
int transcode_dcm_with_filter()
{
    ...
    auto output = "e:\\03_work\\transcode\\mpeg-2_with_filter.mp4";
